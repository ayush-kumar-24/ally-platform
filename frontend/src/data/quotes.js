/**
 * The fallback lines, grouped by the part of the day they suit.
 *
 * SECOND CHOICE NOW, not the only one. The backend picks each founder's lines
 * from a far larger catalogue overnight (backend/app/quotes/catalogue.py); this
 * list is what the card shows when that has not happened -- a failed request, a
 * founder who signed up an hour ago, the feature switched off. It stays bundled
 * rather than fetched so the card is never blank and never a spinner.
 *
 ...
 *
 * NOBODY IS ATTRIBUTED, here or in the backend catalogue. The card renders the
 * line alone: the purpose is recognition -- a founder reading it and thinking
 * "that is my week" -- not authority, and a name under a line is the one way
