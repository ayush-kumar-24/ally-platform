 ...
 *
 *  Deliberately NOT a `maxLength` on the input: a hard cap gives no reason
 *  for the cut-off, so the length is reported below the field instead. */
const TASK_TITLE_MAX = 200;

/** The titles a given box of text would become. One task per submit since
 *  tasks are only ever entered by hand, but kept as a list so the submit handler and
 *  the live over-length hint keep sharing one definition. */
const titlesFrom = (text) => [text.trim()].filter(Boolean);
...
function PlanYourDayInner() {
  ...
  /* Every task used to be created medium with no way to set anything else,
     even though the badge and the sort-by-priority logic both already
     supported all three. */
  ...
  const handlePlanMyDay = async () => {
    ...
    const titles = titlesFrom(text);

    // Checked BEFORE anything is sent or the box is cleared. Reproduced live:
    // an over-long title was rejected by the API with a 422 and what surfaced
    ...
    try {
      ...
      // Every task lands on the day being viewed.
  ...
  /* The title currently over the API's per-task cap, or undefined. Computed
     the SAME way the submit handler computes it, so what the founder is warned
     about is exactly what would have been rejected. */
  const overLongTitle = titlesFrom(inputText).find(t => t.length > TASK_TITLE_MAX);
    ...
    <div className="pad plan-wrap">
      ...
      <div className="plan-grid stagger d3">
        <div className="plan-main">
          <div className="pl-input-card stagger d2" style={{ gap: '12px', marginTop: 24 }}>
            <div className="pl-input-header">
              <span className="pl-input-title">Add a task</span>
            </div>
            ...
            {/* Optional time of day. Without one the task still syncs -- the
                ...
                actually needs it. (There is one entry path: a
            ...
            {/* The over-length warning used to live in the removed Ally
                panel's footer. The per-task cap still applies here, so
                without this the founder only learns the title is too long
                from a toast after pressing Add -- which is the round trip
                the check exists to avoid. */}
            {overLongTitle && (
              <div className="pl-input-hint" role="status">
                <span className="dot" />
                {`That's ${overLongTitle.length} characters — the limit is ${TASK_TITLE_MAX}. Try shortening it.`}
              </div>
            )}
          </div>
...
export default function PlanYourDay() {
  return (
    <PlanGate
      ...
      message="Upgrade to Starter to set your day's priorities and get reminders to stay on them."
    >
      <PlanYourDayInner />
    </PlanGate>
  );
