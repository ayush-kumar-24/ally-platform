...
export default function LandingPage() {
  ...
  /*
   * There used to be a second, self-contained "login" here: a consent drawer
   * that hardcoded the founder as 'Ayush Sharma' <ayush@brightloom.in>, never
   * called any auth provider or the backend, and navigated straight into
   * /guided/welcome with no token. Two things followed from that:
   *
   *   1. It duplicated /guided/login's real consent screen -- a founder saw
   *      what looked like the same sign-up step twice.
   *   2. Because it never obtained a session, GuidedLayout's auth guard
   *      bounced straight back to /guided/login the moment it navigated,
   *      producing the loop this comment replaces: fake drawer -> fake
   *      identity -> guarded route -> kicked back to login -> repeat.
   *
        ...
        <div className="lp-wrap final-in">
          ...
          <div className="lp-reveal d3">
            <span className="cta-wrap">
              <button className="btn btn-emerald btn-lg btn-sheen magnetic" onClick={goToLogin} type="button">
