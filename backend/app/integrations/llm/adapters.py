...
class OpenAIAdapter(RequestAdapter):
    ...
    def build_payload(self, request, model, options):
        payload: dict = {
            ...
            # `max_tokens` is deprecated and is now REJECTED outright by the
            # GPT-5 family: "Unsupported parameter: 'max_tokens' is not supported
            # with this model. Use 'max_completion_tokens' instead." (HTTP 400).
            # FailoverLLMProvider swallows that 400 and silently falls through to
            # MockLLMProvider, so every Ally chat reply came back as the canned
            # "Grounded answer from <model>." template while the routing metadata
            # still named the real model -- a failure that looks like a working
            # integration. `max_completion_tokens` is accepted by both the older
            # (gpt-4o*) and newer models, so it is sent unconditionally.
