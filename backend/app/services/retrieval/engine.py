...
class RetrievalEngine:
    ...
    def search_by_vector(
        ...
        *,
        ...
        for source in selected:
            ...
            results.extend(self._search_one(spec, qvec, k, min_sim))
        return results

    # --- Internals --------------------------------------------------------

    def _search_one(
        self, spec: SourceSpec, qvec: str, k: int, min_sim: Decimal
    ) -> list[RetrievalEvidence]:
        sql = self._build_sql(spec)
        rows = self.db.execute(
            text(sql), {"qvec": qvec, "k": k, "min_sim": min_sim}
        ).mappings().all()
    ...
    def _build_sql(self, spec: SourceSpec) -> str:
        meta_inner = "".join(
