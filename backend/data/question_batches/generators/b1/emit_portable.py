    ...
    for n, (entry, (pkey, dim)) in enumerate(zip(entries, DIM_ORDER), start=1):
        ...
        for stage in STAGES:
            for k, (qtext, qtype, red, green) in enumerate(qs[stage], 1):
                qcode = f"{SPREFIX[stage]}-{pfx}-2{n:02d}-{k}"
...
path = os.path.join(B, "ally_batch1_PORTABLE_no_ids.sql")
open(path, "w").write("\n".join(out) + "\n")
print("written:", path)
